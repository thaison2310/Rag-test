...
def to_text_document(item: Dict) -> Dict:
    ...
    if coll == "products":
        ...
        metadata.update({"id": str(doc.get("_id")), "price": doc.get("price"), "brand": doc.get("brand"), "category": doc.get("categories")})
    # elif coll == "comments":
    #     # Comment text, sender, product reference, rating
    #     text_parts.append(str(doc.get("comment", "")))
    #     text_parts.append(f"Product ID: {doc.get('productId', 'N/A')}")
    #     text_parts.append(f"Sender ID: {doc.get('senderId', 'N/A')}")
    #     text_parts.append(f"Rating: {doc.get('rate', 'N/A')}/5")
    #     text_parts.append(f"Order ID: {doc.get('orderId', 'N/A')}")
    #     metadata.update({"senderId": doc.get("senderId"), "productId": doc.get("productId"), "id": str(doc.get("_id")), "rate": doc.get("rate")})
    elif coll == "vouchers":
